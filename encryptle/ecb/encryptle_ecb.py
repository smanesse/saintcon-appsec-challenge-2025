...
def write_score(score):
    with open(os.path.join(SHARED_DIR, "encryptle_ecb_score.txt"), "wt") as f:
        f.write(str(score))
...
@app.route("/")
def ecb():
    with open("ecb.html", "rt") as f:
        html = f.read()
    solution = random_line("6.txt")
    d = {"word": solution.lower(),
         "ts": int(datetime.now(timezone.utc).timestamp()),
         "nonce": generate_nonce()}
    blob = json.dumps(d)
    ciphertext = encrypt_ecb(blob)
    
    # Write solution to shared directory in test mode
    if os.environ.get('TEST_MODE'):
        try:
            with open(os.path.join(SHARED_DIR, "solution.txt"), "wt") as f:
                f.write(solution.lower())
...
def ecb_check():
    ...
    solution = decrypt_ecb(request.cookies.get("answer"))
    try:
        ...
        if check_time(solution['ts']) and check_nonce(solution['nonce']):
            ...
            if num_tries == 5 or score == 3 * 6:  # max score
