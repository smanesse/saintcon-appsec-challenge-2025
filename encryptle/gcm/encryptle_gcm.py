...
def write_score(score):
    with open(os.path.join(SHARED_DIR, "encryptle_gcm_score.txt"), "wt") as f:
        f.write(str(score))
...
@app.route("/")
def gcm():
    with open("gcm.html", "rt") as f:
        html = f.read()
    solution = random_line("5.txt")
    d = {"word": solution,
         "ts": int(datetime.now(timezone.utc).timestamp()),
         "nonce": generate_nonce()}
    blob = json.dumps(d)
    ciphertext = encrypt_gcm(blob)
...
def gcm_check():
    ...
    solution = decrypt_gcm(request.cookies.get("answer"))
    try:
        ...
        if check_time(solution['ts']) and check_nonce(solution['nonce']):
            ...
            if num_tries == 5 or score == 3 * 5:  # max score
